...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function processExcelData(rawData) {
            ...
            try {
                ...
                async function fetchInChunks(column, values) {
                    ...
                    for (let i = 0; i < total; i += chunkSize) {
                        ...
                        // สร้างกองทัพ SKU: รวมทั้งต้นฉบับ ตัวพิมพ์ใหญ่ และตัวพิมพ์เล็ก เพื่อให้ .in() หาเจอแน่นอน
                        ...
                        if (error) {
                            log(`❌ ดึงข้อมูล ${column} ผิดพลาด: ${error.message}`, 'error');
                            continue;
                        }
                        if (data) {
                            log(`📥 โหลดข้อมูลจาก Master สำเร็จ: +${data.length} รายการ`);
                ...
                if (skusToFetch.length > 0) {
                    ...
                    skusData.forEach(item => {
                        if (item.product_code) {
                            // ใช้ key ที่คลีนที่สุด: เล็กทั้งหมด และไม่มีช่องว่าง
                ...
                const foundKeys = Object.keys(masterDict);
                log(`✅ พบข้อมูลที่แมตช์ได้ในระบบ: ${foundKeys.length} รายการ`, "info");
                if (foundKeys.length > 0) {
                    const sample = foundKeys.slice(0, 3).map(k => k.split('_')[1]).join(', ');
                    log(`ตัวอย่างข้อมูลที่พร้อมแมตช์: ${sample}`, "success");
                }
            } catch (err) {
        ...
        rawData.forEach((row, index) => {
            ...
            let orderNumber = `UNK-${index}`, trackingNumber = null, courierPhone = null, latestLocation = null, bookImageUrl = null, productCode = null, productName = "Unknown", quantity = 1, price = 0, shipping = 0, buyer = "Unknown", address = null, province = "Unknown", status = "รอดำเนินการ", note = null;
            if (platform === 'Lazada') {
                ...
                bookImageUrl = getValue(row, ['book_image_url', 'รูปภาพ', 'รูปใบ Book', 'Image', 'Photo', 'URL']);
                courierPhone = getValue(row, ['courier_phone', 'เบอร์ขนส่ง', 'เบอร์โทรขนส่ง', 'Courier Phone']);
                latestLocation = getValue(row, ['latest_location', 'สถานที่ล่าสุด', 'สาขาล่าสุด', 'Location']);
                ...
                bookImageUrl = getValue(row, ['book_image_url', 'รูปภาพ', 'รูปใบ Book', 'Image', 'Photo', 'URL']);
                courierPhone = getValue(row, ['courier_phone', 'เบอร์ขนส่ง', 'เบอร์โทรขนส่ง', 'Courier Phone']) || courierPhone;
                latestLocation = getValue(row, ['latest_location', 'สถานที่ล่าสุด', 'สาขาล่าสุด', 'Location']);
            ...
            if (master) {
                log(`🎯 แมตช์ SKU [${productCode}] สำเร็จ! กำลังดึงข้อมูลจาก Master...`, 'success');
                if (!productCode) productCode = master.product_code; 
                ...
                if (productCode) log(`🔍 ไม่พบข้อมูล Master สำหรับ SKU: ${productCode}`, 'info');
            }
    ...
    if (bulkUploadBtn) {
        bulkUploadBtn.addEventListener('click', async () => {
            ...
            try {
                // 1. ตรวจสอบเลขพัสดุซ้ำเท่านั้น (order_number ซ้ำได้)
                const trackingNumbers = parsedDataToUpload.map(o => o.tracking_number).filter(t => t);
                
                log(`กำลังค้นหาเลขพัสดุที่มีอยู่แล้วในระบบ...`);
                let existingTrackingSet = new Set();
                if (trackingNumbers.length > 0) {
                    ...
                    if (checkError) throw checkError;
                    existingTrackingSet = new Set(existing.map(e => String(e.tracking_number)).filter(t => t !== 'null'));
                }

                // 2. กรองข้อมูลเอาเฉพาะรายการที่เลขพัสดุไม่ซ้ำ
